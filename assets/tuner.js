// tuner.js — adaptive gate + YIN with zero-cross fallback + relaxed filters
import {
  rms,
  detectPitchYIN,
...
const courseEls = COURSES.map((c, idx) => {
  ...
  div.innerHTML = `
    ...
    <div class="pill"></div>
    <div class="label">
      <div class="main">${c.name}</div>
      <div class="sub">${c.hz.toFixed(1)} Hz</div>
    </div>
  `;
...
// Tunables
const DETECT_MS = 45;   // pitch detect cadence
const HOLD_MS   = 1200; // hold last reading after silence
let   holdUntil = 0;
let   lastStable = null;

// Adaptive gate
let noiseFloor = 0;     // tracked when we're not "audible"
function audibleGate(r){
  // dynamic threshold: 3x noise floor, floored at 0.003
  const thr = Math.max(0.003, noiseFloor * 3);
  return r > thr;
}
...
async function start(){
  ...
  // Relax filters a bit: HP 20 Hz, LP 3000 Hz
...
function stop(){
  ...
  ac = analyser = src = null;
  stream = null; workBuf = null;
  holdUntil = 0; lastStable = null; noiseFloor = 0;
  ...
  centsBig.textContent='0.0'; centsBig.className='cents-big';
...
function loop(){
  ...
  const audible = audibleGate(level);
  if (!audible) {
    // Track noise floor (EMA) when not audible
    noiseFloor = noiseFloor === 0 ? level : (noiseFloor*0.9 + level*0.1);
  }
  ...
  if ( (now - detectLast) >= DETECT_MS ){
    ...
    if (audible) {
      // Primary: YIN
      hz = detectPitchYIN(workBuf, ac.sampleRate);
      // Fallback if YIN fails this frame
      if (!hz) hz = detectPitchZeroCrossing(workBuf, ac.sampleRate);
    }
    ...
    if (hz){
      ...
      // Hold last reading while sound fades
